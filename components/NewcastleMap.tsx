...
const NewcastleMap: React.FC = () => {
  ...
  return (
    <div className="relative w-full aspect-[4/3] md:aspect-[2/1] bg-slate-900/50 rounded-[2rem] md:rounded-[2.5rem] border border-white/5 overflow-hidden group">
      ...
      <svg className="absolute inset-0 w-full h-full p-6 md:p-20 opacity-30 group-hover:opacity-50 transition-opacity" viewBox="0 0 800 400" preserveAspectRatio="xMidYMid slice">
      ...
      {landmarks.map((point, i) => (
        ...
        >
          ...
          <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 opacity-0 group-hover/point:opacity-100 transition-all pointer-events-none whitespace-nowrap z-30 scale-75 md:scale-100">
      ...
      <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 text-right">
        <h4 className="text-white font-black text-xs md:text-xl mb-1">Local Impact</h4>
        <p className="text-[8px] md:text-[10px] text-blue-500 font-black uppercase tracking-widest">Newcastle KZN Infrastructure</p>
      </div>

      <div className="absolute top-4 left-4 md:top-8 md:left-8 flex items-center space-x-2">
         <div className="w-1.5 h-1.5 md:w-2 md:h-2 bg-emerald-500 rounded-full animate-pulse"></div>
         <span className="text-[8px] md:text-[10px] text-emerald-500 font-black uppercase tracking-widest">48 Active Projects</span>
      </div>
    </div>
  );
};
