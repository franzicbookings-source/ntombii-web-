...
const Services: React.FC = () => {
  return (
    <div className="pt-24 md:pt-32 pb-20 md:pb-24 text-slate-200">
      <SEO 
        title="Services | NTOMBII TECH - Web Design & Digital Strategy"
        description="Comprehensive digital services including high-performance web development, local cloud hosting, SEO optimization, and AI automation for Newcastle businesses."
        keywords="web services Newcastle, local hosting South Africa, SEO services KZN, automation studio South Africa"
      />
      ...
      <section className="container mx-auto max-w-6xl px-4 md:px-6 mb-24 md:mb-40">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
          {CORE_SERVICES.map((service, idx) => (
            <ScrollReveal key={service.id} delay={idx * 150} animation={idx % 2 === 0 ? 'right' : 'left'}>
              <div className="glass-card flex flex-col rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden group relative h-full">
                ...
                <div className="h-56 bg-gradient-to-br from-blue-600/10 to-transparent flex items-center justify-center p-12 relative overflow-hidden">
                  <div className="text-blue-500 transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-700">
                     {React.isValidElement(ICON_MAP[service.icon]) ? React.cloneElement(ICON_MAP[service.icon] as React.ReactElement, { size: 64, strokeWidth: 1.2 } as any) : null}
                  </div>
                </div>
        ...
        <div className="container mx-auto max-w-6xl px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
             <ScrollReveal animation="right">
               <div className="glass-card p-8 md:p-14 rounded-[3rem] border-red-500/10 bg-red-500/[0.02] h-full">
             ...
             <ScrollReveal animation="left" delay={200}>
               <div className="glass-card p-8 md:p-14 rounded-[3rem] border-blue-500/30 bg-blue-600/[0.05] relative overflow-hidden h-full">
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-center">
               ...
               <div>
                  ...
                  <div className="space-y-8">
                     {[
                        ...
                        <ScrollReveal key={i} delay={i * 150} animation="left">
                          <div className="flex items-start space-x-6">
                             <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-blue-400 shrink-0">
                                {React.isValidElement(item.icon) ? React.cloneElement(item.icon as React.ReactElement, { size: 20 } as any) : null}
                             </div>
                             <div>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {ADDITIONAL_SERVICES.slice(0, 4).map((service, i) => (
                  <ScrollReveal key={i} delay={i * 100} animation="up">
                    <div className="glass-card p-8 rounded-[2.5rem] border-white/5 flex flex-col items-center text-center h-full">
      ...
      <section className="py-24 md:py-40 bg-white/[0.01]">
        <div className="container mx-auto px-6">
          ...
          <ScrollReveal delay={200} animation="up">
            <FAQSection />
          </ScrollReveal>
        </div>
      </section>
      ...
      <section className="py-24 md:py-40 bg-slate-950 border-t border-white/5">
        <div className="container mx-auto max-w-6xl px-4 md:px-6">
          <ScrollReveal>
            <div className="text-center mb-20">
              <h3 className="text-3xl md:text-7xl font-black text-white mb-6">Standard <span className="text-gradient">Tiers</span></h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
            {PRICING_TIERS.map((tier, idx) => (
              <ScrollReveal key={tier.id} delay={idx * 150} animation="up" className="h-full">
                <div className={`relative glass-card p-8 md:p-14 rounded-[3rem] flex flex-col h-full ${tier.recommended ? 'border-blue-500/50 bg-blue-600/5 shadow-[0_0_100px_rgba(37,99,235,0.1)]' : ''}`}>
                  {tier.recommended && (
